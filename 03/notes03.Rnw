...
Let's write the \myemph{least squares equations} \myref{\LSi} in matrix form.

\end{frame}


\begin{frame}[fragile]
\frametitle{The least squares equations in matrix form for simple linear regression}
