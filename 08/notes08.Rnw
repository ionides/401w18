...
\begin{frame}[fragile]
\frametitle{Colinear explanatory variables in a linear model}


\begin{myitemize}
\item Let \m{\mat{X}=[x_{ij}]_{n\times p}} be an \m{n\times p} design matrix. 
\item If there is a nonzero vector \m{\vect{\alpha}=(\alpha_1,\dots,\alpha_p)} such that \m{\mat{X}\vect{\alpha}=\vect{0}} then the columns of \m{\mat{X}} are \myemph{colinear}.
...
\item Colinearity of explanatory variables has important consequences for fitting a linear model to data.
\item It can also be useful to notice whether the variables are close to colinear, meaning that  \m{\mat{X}\vect{\alpha}} is small but nonzero.
\end{myitemize}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{The least squares fit with colinear predictors}
...
\item Suppose that \m{\mat{X}} is colinear, with \m{\mat{X}\vect{\alpha}=\vect{0}}.
\item Since
\mydisplaymath{
...
\item
\myemph{When \m{\mat{X}} is colinear, a least squares coefficent still exists, but it is not unique.}
...
\begin{frame}[fragile]
\frametitle{Standard errors for colinear variables}

\myquestion. Any variable that is part of a colinear combination of variables has infinite standard error. Why?
...
\begin{frame}[fragile]
\frametitle{What does R do if give it colinear variables?}
<<>>=
...
\begin{myitemize}
\item R noticed that the three explanatory variables are colinear, and refused to fit the third
\end{myitemize}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{Colinear variables and the determinant of $\mat{X}^\transpose\mat{X}$}

\begin{myitemize}
\item Recall that the variance of \m{\hat\beta} in the usual linear model is \m{\sigma^2 \big(\mat{X}^\transpose\mat{X}\big)^{-1}}.
\item Colinearity means the variance is infinite, a matrix version of dividing by zero.
\item Recall that a square matrix is invertible if its determinant is nonzero.
\item We can check that colinearity means \m{\mathrm{det}\big(\mat{X}^\transpose\mat{X}\big)=0}.
<<>>=
X <- model.matrix(lm1)
t(X)%*%X
det(t(X)%*%X)
...
\begin{myitemize}
\item Columns of a matrix that are not colinear are said to be \myemph{linearly independent}.
...
\item If \m{\mat{X}} does not have full rank, we can drop \myemph{linearly dependent} columns until the remaining columns are linearly independent. This is a practical approach to handling colinearity.
\end{myitemize}
...
\begin{frame}[fragile]
\frametitle{Almost colinear variables}
