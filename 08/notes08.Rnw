...
\begin{frame}[fragile]
\frametitle{R-squared and adjusted R-squared}
...
\myquestion. Write the hypotheses \m{H_0} and \m{H_1} to match the \m{R^2} statistic in a linear model with \m{p} explanatory variables (including the intercept).
...
\item Further, we see that \m{\RSS_0} is \m{\SST}, and \m{\RSS_a} is \m{RSS}.
\end{myitemize}
