...
\myquestion. Similarly, show that \m{\cov\big(Y,\sum_{j=1}^n Z_j\big)=\sum_{j=1}^n\cov(Y,Z_j)}.
...
\myquestion. Show that \m{\cov(Z_i,Z_j) = \sum_{k=1}^n\sum_{\ell=1}^n a_{ij}a_{k\ell}[\mat{V}_Y]_{k\ell}}
...
\framebox{
\m{\var(\mat{A}\vect{Y})
= \mat{A}^\transpose \var(\vect{Y}) \mat{A}}.
\rule[-3mm]{0mm}{9mm}
}
\end{myitemize}
\myquestion.
...
\item Let's first check how to extract the estimates and their standard errors from R. We can visually read them off the summary, but that's not satisfactory as a step in a reproducible research workflow.
\end{myitemize}
