...
For a more complete exploration, I graphed the $T$ distribution on 10 degrees of freedom, $T^2$ and the $F$ distribution. You were not expected to do this.
...
t = matrix(c(seq.int(-3*m, 3*m)))
colnames(t) <- "t"
t = t/m
t <- as.data.frame(t)
t$prob <- dt(t$t, 10)

plot(t$t, t$prob, main = "T distribution with 10 df", xlab = "t", ylab = "Density", type = "l")

t$t2 <- t$t^2
t$t2_prob <- dt(t$t, 10)
t$f_prob <- df(t$t2, 1, 10)

f <- t[, 3:5]
f <- unique(f)

ggplot() + geom_line(aes(x= f$t2, y = f$t2_prob)) + ylim(0,0.4) + labs(x = "t^2", y = "Density", title = "T distribution with t^2 and 10 degrees of freedom")
ggplot() + geom_line(aes(x= f$t2, y = f$f_prob)) + ylim(0,0.4) + labs(x = "t^2", y = "Density", title = "F distribution with t^2 and (1, 10) degrees of freedom")
...
hist(Tsim,freq=F,main="", breaks=30,ylim=c(0,0.4))
x <- seq(length=200,min(Tsim),max(Tsim))
...
where $f$ ~ $F_{4, 698}$. The p-value was calculated as the $P(f^* > f = 0.824)$. Since the p-value is equal to 0.505 we fail to reject the null hypothesis at an $\alpha = 0.05$. The ANOVA agrees with what we saw in the regression output: the year a student enters college is not signficant.
