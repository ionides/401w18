...
To understand the code provided below, note that if $T$ follows the t-distributoin (with n degrees of freedom), then $T^2$ follows the F Distribution (with 1 and n degrees of freedom). Note that, if $T^2<x$ this means that $ -\sqrt{x} < T < \sqrt{x}$. This means that pf(x,1,n) = $P(T^2<x) = P(-\sqrt{x}<T<\sqrt{x}) = P(T< \sqrt{x}) - P(T< - \sqrt{x})$ = pt($\sqrt{x}$,n) - pt($-\sqrt{x}$,n). Let us confirm this using R.
We will compare the probability values from the two distributions for a series of values for X. Fix the degrees of freedom of the t-distribution to be any number (n=10 in this case).
