...
To write model (LM) in the form $\vec{y} = \mat{X}\vec{b} + \vec{e}$, we need $\mat{X}=[\vec{x},\vec{z_1},\dots,\vec{z_{19}}]$ where $\vec{x}$ is a column vector containing $(x_1,\dots,x_{76})$ and $\vec{z_k}$ is a column vector containing $(z_{1,k},\dots,z_{76,k})$ for $k=1,\dots,19$.
