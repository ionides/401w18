...
This is because some of the covaraites are almost collinear. For example, height with shoes and height without shoes almost model the same things. Thus the determinant of $\mathbb{X}^{\top} \mathbb{X}$ is close to zero and the variance of the least squares coefficient vector becomes large.
...
The standard error of the coefficients of height of person will probabily decrease, since we do not have the collinearity in here.
...
$(4.30-1.96*1.92,4.30+1.96*1.92)$

4. How is the residual standard error calculated for this model? (Give a formula.)

Solution:

$s = \sqrt{\frac{\sum_{i=1}^n(y_i-\hat{y_i})^2}{n-p}}$, where $\hat{y_i}=x_i^{\top}b$ is the fitted value of $y_i$ and $p$ is the number of parameter we have in this model.
...
Plot the number qq-plot of the residuals. Check if the points roughly follows a straight line.
...
Under $H_0$, we have the model $Y_i = \beta_0 + \beta_1 x_{i1} + \beta_2 x_{i2}+...+\beta_6 x_{i6} + \epsilon_i$,$i=1,...,n$. Where $Y_i$ as the birth weight for obervation i, $x_{i1}$ is the parents' socioeconomic status while $x_{ij}$, $j=2,...6$ is the indicator for parents' education level
.  $\epsilon_i$ are i.i.d with mean 0 and variance $\sigma^2$.

Under $H_a$, we have $Y_i = \beta_0 + \beta_1 x_{i1} + \beta_2 x_{i2}+...+\beta_6 x_{i6} +  \beta_7 x_{i1}x_{i2} +...+\beta_{11}x_{i1}x_{i6} +  \epsilon_i$,$i=1,...,n$.

For the F test, we are testing 

$H_0:\beta_7=...+\beta_{11}=0$ against $H_a$: one of $\beta_j\neq 0$, $j=7,...,11$
...
The row vectors in a) are linearly independent while the row vectors in b) are linearly dependent.
