...
\begin{frame}
\frametitle{Linear algebra for applied statistics}
...
\question: You may or may not have seen vectors in other contexts. In physics, a vector is a quantity with magnitude and direction. How does that fit in with our definition?

\vspace{25mm}

\question: How can I distinguish vectors using handwriting, when a bold font is not possible?
...
\begin{frame}[fragile]
\frametitle{Adding vectors and multiplying by a scalar: Example}
\begin{myitemize}
\item
...
%\myexample: 
An ecologist measures the pH of ten Michigan lakes at two points in the summer. Set up vector notation to describe her data. Write a vector calculation to find the average pH in each lake.
...
this is \mymath{\vec{\mu} = \frac{1}{2}(\vec{x}+\vec{y})}.
}
\end{frame}
...
\question: Why does R break the usual rules of mathematics here?
...
is exactly equivalent to the collection of \m{p} linear equations in (L1) above.
\item
Mechanically, the \m{i}th component of \m{\mat{A}\vec{x}} is found by multiplying each term in the \m{i}th row of \m{\mat{A}} with corresponding terms in the \myemph{column vector} \m{\vec{x}}, and adding these contributions.  See homework for practice!
\end{myitemize}
...
\item The general linear system we wrote previously in (L1) has \m{n} equations with \m{p} unknowns. 
...
\item Is there a \myemph{matrix inverse} \m{\mat{A}^{-1}} such that \m{\vec{x}=\mat{A}^{-1}\vec{b}}?
...
\question: Let \m{\mat{U}=\left[\begin{array}{cc} 2& 2 \\ 1 & -1 \end{array}\right]} and
...
\begin{myitemize}
\item
Scalar multiplication (i.e., the usual multiplication of two real numbers) has the \myemph{commutative} property, \m{uv=vu}.
\item
...
\question. Suppose \m{\mat{B}} is a \m{n\times q} matrix and \m{\mat{I}} is an identity matrix.
...
\item We can solve a pair of linear equations by hand. First, we solve for \m{x} by eliminating \m{y}. We can rewrite (L2) as 
...
bcx &+& bdy &=& bv
\end{array}
\hspace{20mm}\begin{array}{c} (\mathrm{L3.1}) \\ (\mathrm{L3.2}) \end{array}
}
\item
Subtracting (L3.2) from (L3.1) gives \m{(ad-bc)x = du-bv} and so \m{x=\frac{1}{ad-bc}(du-bv)}. 
\end{myitemize}
\end{frame}
...
\begin{myitemize}
\item
Next, we can find \m{y} by eliminating \m{x}. We rewrite (L2) as
\mydisplaymath{
\begin{array}{ccccc}
acx &+& bcy &=& cu \\
acx &+& ady &=& av
...
\item We can see from the formula for \m{ \mat{A}^{-1}} that the inverse of $\mat{A}$ exists if and only if \m{ad-bc \neq 0}.
\end{myitemize}
\question: What does it mean geometrically for \m{ad-bc = 0}?

\hspace{10mm}
Hint: the slope of the line \m{ax+by=u} is \m{-a/b}. The slope of \m{cx+dy=v} is \m{-c/d}. 
...
%Numerical inaccuracies may arise, especially when the determinant is close to zero.
\end{myitemize}
...
\question: Why is \url{M %*% M_inv} not exactly equal to the identity matrix?
...
1. Write the system as a matrix equation $\mat{A}\vec{x}=\vec{b}$,
...
\question: Which of these correctly computes \m{\vec{x}} and why?
\begin{columns}[T] 
\begin{column}{0.4\textwidth}
<<>>=
round(solve(A) %*% b,2)
...
\item The \myemph{transpose} of \m{\mat{A}} is denoted mathematically by \m{\mat{A}^\transpose_{}} and in R by \code{t(A)}.
\end{myitemize}
\mydisplaymath{
\mat{A}=\left[
\begin{array}{ccc}
...
\begin{myitemize}
\item
If \m{\mat{A}} has dimension \m{n\times p}, then \m{\mat{A}^\transpose_{}} is \m{p\times n}.
\end{myitemize}
\end{frame}
