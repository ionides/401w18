...
\end{myitemize}
\altdisplaymath{(LM1) \hspace{10mm}
...
\item 
The \myemph{residual error} terms \m{e_1,\dots,e_n} in equation \m{(LM1)} include everything about the data \m{y_1,y_2,\dots,y_n} that cannot be explained by the \myemph{linear combination} of the explanatory variables.
\item 
...
\begin{myitemize}
\item When we did \code{L_fit <- lm(Total~Year,data=L)} earlier, we fitted the linear model \m{(LM1)} with \m{y_i} being the total life expectancy for the \m{i}th year in the dataset (recall that total life expectancy means combining males and females) and \m{x_{i1}} being the corresponding year.
\item 
To fit a linear trend, we also want an \myemph{intercept}, which we can write by setting \m{x_{i2}=1} for each year \m{i}. 
\item 
In this special case, with \m{p=2} variables and \m{x_{i2}=1}, the model \m{(LM1)} becomes

\altdisplaymath{(LM3) \hspace{10mm}
