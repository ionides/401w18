...
{\bf Pre-requisites}: We will assume familiarity with material in STATS 250. All course notes and labs are at
...
We say \mymath{L} is a matrix with \myemph{dimension} \mymath{m\times n}. To get the dimension in R,
<<dim>>=
dim(L)
@
...
\begin{frame}[fragile]
\frametitle{Vectors}
A single row or column of a matrix is called a \myemph{vector}. For example, we can set \code{y} to be total life expectancy, combining men and women,
<<vec>>=
y <- L[,4]
y[1:3]
@
...
Mathematically, we write \altdisplaymath{y=\left[
\begin{array}{c}
y_{1}\\
y_{2}\\
\vdots \\
...
We see that \mymath{y} is a \mymath{m\times 1} matrix. 
We call \mymath{y} a \myemph{column vector}.
A row of \mymath{L} is a \mymath{1\times n} matrix called a \myemph{row vector}.
...
%The length of the logical vector should match the length of the vector subsetted, or the dimension of a matrix being subsetted.
