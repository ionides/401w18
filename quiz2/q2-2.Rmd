...
(a) Specify a row matrix $\mathbf{x}^*$ so that $\hat y^*=\mathbf{x}^*\mathbf{b}$ gives the least square predictor.

(b) Write an expression for the variance of $\hat Y^*=\mathbf{x}^*\mathbf{\hat\beta}$ where $\mathbf{\hat\beta}$ is the least squares fit on model-generated data, i.e., $\mathbf{\hat\beta}=(\mat{X}^\transpose\mat{X})^{-1}\mat{X}^\transpose \vect{Y}$.
